...
with st.sidebar:
    ...
    st.divider()
    # زرار الواتساب (تم إرجاعه وتنسيقه)
...
st.divider()
st.subheader("🤖 منظم الجدول بالذكاء الاصطناعي")
ai_col1, ai_col2 = st.columns([1, 2])

with ai_col1:
    st.write("بعد إضافة مهامك بالأسفل، اضغط هنا لتنظيم وقتك:")
    if st.button("🪄 توليد جدول مذاكرة ذكي"):
        ...
        if active_tasks:
            ...
            for task in active_tasks:
                ...
                plan.append(f"📖 **{start_str} - {end_str}**: مذاكرة ({task})")
    ...
    for i, t in enumerate(st.session_state.tasks):
        with st.container():
            ...
            if is_done != t['done']:
                ...
                st.rerun()
            txt = f"~~{t['name']}~~" if t['done'] else f"**{t['name']}**"
            tc2.write(txt)
...
st.divider()
st.caption(f"Study Flow AI Ultimate v2.5 | 2026 | نقاطك الإجمالية: {st.session_state.xp}")
